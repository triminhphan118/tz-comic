...
const ChappterHead: FC<ChappterHeadProps> = ({ chaptersData, chap, listChapter, numberChap }) => {
    ...
    return (
        <>
            <div className={`w-full flex-col ${isSticky ? 'sticky top-0 z-50' : ''}`}>
                <div className={`flex gap-4 items-center flex-col md:flex-row ${isSticky ? 'hidden' : ''}`}>
                    <h3 className="underline text-blue-500">{chaptersData.item.comic_name}</h3>
                    {/* <span>[Cập nhật lúc: 13:46 21/11/2022]</span> */}
                </div>
                <div
                    className={`flex flex-col justify-center w-full text-center mt-2 bg-gray-100 rounded-sm p-4 ${
                        isSticky ? 'hidden' : ''
                    }`}
                >
                ...
                <span
                    className={`flex gap-1 items-center justify-center mt-4 bg-blue-100 rounded-sm p-4 ${
                        isSticky ? 'hidden' : ''
                    }`}
                >
                    <RiErrorWarningFill />
